...
void run_matrix_test(char* mtx_path, char* vec_path, float * std_res, float * gc_res[], float * bc_res[], float * dg_res[]) {
    HPCGMatrix A;
    FloatVector v;
    float maxValue;
    ...
    InitializeFloatVectorFromPath(v, vec_path, A.numberOfColumns);
    ...
    for(int i=0; i<4; i++) {
        ...
        gc_res[i][1] = gc_time;
        gc_res[i][2] = gc_iter;
        gc_res[i][3] = ((float)normal_time)/(gc_time+coloring_ovh);
        gc_res[i][4] = th_speedup;
        gc_res[i][5] = ((float) coloring_ovh)/normal_time;
        num_threads *= 2;
    }
    ...
    for(int i=0; i<4; i++) {
        ...
        bc_res[i][1] = bc_time;
        bc_res[i][2] = bc_iter;
        bc_res[i][3] = ((float)normal_time)/(bc_time+block_coloring_ovh);
        bc_res[i][4] = th_speedup;
        bc_res[i][5] = ((float) block_coloring_ovh)/normal_time;
        num_threads *= 2;
    }
    ...
    for(int i=0; i<4; i++) {
        ...
        dg_res[i][1] = dg_time;
        dg_res[i][2] = dg_iter;
        dg_res[i][3] = ((float)normal_time)/(dg_time+dep_graph_ovh);
        dg_res[i][4] = th_speedup;
        dg_res[i][5] = ((float) dep_graph_ovh)/normal_time;
        num_threads *= 2;
    }
}
